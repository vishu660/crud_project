          ...
          <div class="row">
            ...
            <div class="card p-0 mt-3">
                ...
                <div class="card-body shadow-lg">
                  <form action="{{ route('products.update' , $product->id) }}" method="post" enctype="multipart/form-data" class="py-3">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                     ...
                     <div class="mb-3">
                        ...
                        @enderror
                           @if (!empty($product->image))
                                   <img src="{{ asset('uploads/products/' . $product->image) }}" alt="Product Image" width="150" class="rounded mt-3">
                                  @endif
                    </div>
                      <div class="mb-3">
                        <label for="sku" class="form-label">SKU</label>
                        <input value="{{ old('sku', $product->sku) }}" type="text" class="form-control @error('sku') is-invalid @enderror" id="sku" name="sku" placeholder="Enter product sku">
                         @error('sku')
                            <p class="invalid-feedack text-danger">{{$message}}</p>
                        @enderror
                    </div>
                      <div class="mb-3">
                        ...
                        <input value="{{ old('price', $product->price) }}" type="text" class="form-control @error('price') is-invalid @enderror" id="price" name="price" placeholder="Enter product price">
                         @error('price')
                            <p class="invalid-feedack text-danger">{{$message}}</p>
                        @enderror
                    </div>
